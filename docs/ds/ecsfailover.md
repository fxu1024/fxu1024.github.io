...
## 3. Shutdown ECS server, delete node and then add it back
...
- The above are all expected behaviors when the ECS server node is down. Pods in terminating state can be removed from the apiserver when the Node object is manually deleted.
...
- Confirm that Node feng-ws1 disappeared.
...
- Stop ECS server role on feng-ws1.
 

- Delete ECS server role on feng-ws1.

 

 

- You'll see the warning This entity is currently running with an outdated configuration. 
...
- Please refer to Common operations after pod eviction to recovery all failed Pods.

- Confirm that all pods are green.

 


- Confirm that ECS service has no alerts except ECS server health.
...
- Please ssh into feng-ws1 and manually clean up rke2 server.
...
- Add feng-ws1 as ECS server role by CM UI
...
- Start ECS role on feng-ws1.
 
 

 

- You'll see the warning This entity is currently running with an outdated configuration.  If you click the refresh icon, the whole ECS cluster will restart.
...
- Confirm that feng-ws1 started successfully.
...
- As you can see from the HAProxy UI, http port 80 &  https port 434 ports on feng-ws1 are UP now.
...
- Confirm that ECS service has no alerts
